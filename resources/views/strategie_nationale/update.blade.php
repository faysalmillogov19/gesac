...
<div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Strategie nationale</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            </div>
            <form class="modal-body" method="POST" action="{{route('strategie_nationale.store')}}">
                @csrf
                ...
                @if($row->id)<input type="hidden" name='id' value="{{$row->id}}" required>@endif
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="code" id="exampleInputPassword1" @if( $row->code ) value="{{$row->code}}" @endif placeholder="Libelle" required>
                  </div>
