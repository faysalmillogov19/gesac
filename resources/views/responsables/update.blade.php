            ...
            <form class="modal-body" method="POST" action="{{route('responsable.store')}}">
                ...
                @if($row->id)<input type="hidden" name='id' value="{{$row->id}}" required>@endif
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="direction" id="exampleInputPassword1" placeholder="Direction" 
                    @if( $row->direction ) value="{{$row->direction}}" @endif required>
                  </div>
                  <div class="form-group">
                    <label for="exampleInputPassword1">description</label>
                    <textarea class="form-control" name="description" placeholder="Description" required>@if($row->description) {{$row->description}} @endif</textarea>
                  </div>
